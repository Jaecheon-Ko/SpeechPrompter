...
- **음성인식**: [sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx) + SenseVoice 다국어 모델 (중/영/일/한/광둥어, int8 약 240MB). 인식 중 인터넷 불필요. 언어 모드(한국어/한영/English) 전환 가능.
- **위치 추적**: 단어 기반 매칭 + 오점프 방지 채택 조건(마지막 단어 필수 일치, 점프 거리별 점수 임계값, 전진만 허용).
- **스크롤**: Choreographer 프레임 루프, 단조 증가 잠금(역주행 금지), 읽기 속도(wps) 기반 선행 보정, 말을 멈추면 스크롤도 감쇠 정지. 수동 모드는 등속 스크롤(10~200px/s).
- **화면**: 3단계 색상(읽은 부분 어둡게/현재 단어 노랑/남은 부분 밝게), 읽기 라인(위치 15~55% 조절), 글자 24~90sp, 미러 모드, 본문 탭으로 컨트롤 바 숨김, 3-2-1 카운트다운, 화면 꺼짐 방지.
- **대본 관리**: txt/docx 파일 불러오기(POI 없이 ZIP+XML 직접 파싱), 저장 목록(최근순), 시작 시 자동 저장.
